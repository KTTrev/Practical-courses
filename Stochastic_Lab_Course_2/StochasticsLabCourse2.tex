...
the same countywide random effect. We can interpret from table \ref{tableau91} that Soybeans tend to have 
	\begin{table}[H]
		...
		\begin{tabular}{|P{2cm}|P{2.5cm}|P{2.5cm}|P{2.5cm}|P{2.5cm}|}
			\hline
			& \multicolumn{2}{c|}{Fix effects} & \multicolumn{2}{c|}{Random effects (StdDev)} \\
			\hline
	...
	\newpage
	\item Now we want to compare and
evaluate with respect to their reliability, in order to obtain predictions for $\mu_i= \bar{x}^t_{ip}\beta+ v_i$. Here, for the $i$th county and a specified
crop, $x_{ip}$ is the population mean of the explanatory variables and $\bar{x}_i$ the mean over the
observed segments only. Also $\hat{\beta}$ is the weighted least-squares estimator for $\beta$ and $\gamma_i= \sigma^2_v(\sigma^2_v+ n^{-1}_i\sigma^2_\epsilon)^{-1}$ where $n_i$ the number of observations in the $i$th county, $\sigma^2_v$ and $\sigma^2_\epsilon$ the variances of random effect and error, respectively.
	\begin{itemize}
	...
	\begin{figure}[H]
		\centering
		\includegraphics[scale=0.5]{Ex9plot1.png}
	...
	\begin{figure}[H]
		\centering
		\includegraphics[scale=0.5]{Ex9plot2.png}
...
\begin{enumerate}[label=(\alph*)]
	...
	\begin{figure}[H]
		...
		\begin{subfigure}[b]{0.48\linewidth}        %% or \columnwidth
			...
			\caption{Plot of the Pearson correlationcoefficients against model dimension}
			\label{fig 10.1}
		\end{subfigure}
		\begin{subfigure}[b]{0.48\linewidth}        %% or \columnwidth
			\centering
			\includegraphics[scale=0.25]{Ex10plot2.png}
			\caption{Plot of the values predicted by the PLSmodel of dimension $d_{opt}$ on the test set against corresponding age values from the test set}
			\label{fig 10.2}
		\end{subfigure}
		\caption{}
		\label{figuuuu}
